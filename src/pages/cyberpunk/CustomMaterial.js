...
class CustomMaterial extends ShaderMaterial {
		...
		super({
			...
			fragmentShader: `
				...
				void main() {
					...
					vec4 cr = texture2D(texture_data, p + offset);
					vec4 cga = texture2D(texture_data, p);
					vec4 cb = texture2D(texture_data, p - offset);
					if (hasTexture == 1.0) gl_FragColor = vec4(cr.r, cga.g, cb.b, cga.a);
					else gl_FragColor = vec4(color, opacity);
				}
			`,
			uniforms: {
				texture_data: { value: null },
				hasTexture: { value: 0 },
				scale: { value: 0 },
				shift: { value: 0 },
				opacity: { value: 1 },
	...
	set map(value) {
		...
		this.uniforms.texture_data.value = value
	}
	get map() { return this.uniforms.texture_data.value }
